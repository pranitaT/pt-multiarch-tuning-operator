...
func (r *PodReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Dump pod state at entry
	nodeSelectorJSON, _ := json.Marshal(pod.Spec.NodeSelector)
	affinityJSON, _ := json.Marshal(pod.Spec.Affinity)
	...
	// Dump complete pod state before update
	nodeSelectorBeforeJSON, _ := json.Marshal(pod.PodObject().Spec.NodeSelector)
	affinityBeforeJSON, _ := json.Marshal(pod.PodObject().Spec.Affinity)
	...
	if pod.PodObject().Spec.Affinity != nil &&
		...
		termCount = len(pod.PodObject().Spec.Affinity.NodeAffinity.RequiredDuringSchedulingIgnoredDuringExecution.NodeSelectorTerms)
		// DIAGNOSTIC: Dump NodeSelectorTerms in detail
		termsJSON, _ := json.Marshal(pod.PodObject().Spec.Affinity.NodeAffinity.RequiredDuringSchedulingIgnoredDuringExecution.NodeSelectorTerms)
		log.Info("[DIAGNOSTIC] BEFORE r.Update() - NodeSelectorTerms",
			"traceID", traceID,
			"pod", pod.Name,
	...
	log.Info("[UPDATE] BEFORE",
		...
		"pod", pod.Name,
		"namespace", pod.Namespace,
		"nodeSelectorTermsCount", termCount,
		"nodeSelector", string(nodeSelectorBeforeJSON),
		"affinity", string(affinityBeforeJSON))

	err := r.Update(ctx, pod.PodObject())
	if err != nil {
		// Dump complete state on failure
		nodeSelectorFailJSON, _ := json.Marshal(pod.PodObject().Spec.NodeSelector)
		affinityFailJSON, _ := json.Marshal(pod.PodObject().Spec.Affinity)

		log.Error(err, "[UPDATE] FAILED",
			"traceID", traceID,
			"pod", pod.Name,
			"namespace", pod.Namespace,
			"nodeSelector", string(nodeSelectorFailJSON),
			"affinity", string(affinityFailJSON))
